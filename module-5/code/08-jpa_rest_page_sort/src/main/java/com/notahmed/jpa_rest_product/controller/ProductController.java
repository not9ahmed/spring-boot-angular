...
public class ProductController {
    ...
    // Adding pagination to the list of products
    @GetMapping("/paginated")
    public ResponseEntity<List<Product>> getAllProductsPaginated(@RequestParam(value = "page", defaultValue = "0") int page,
                                                                 @RequestParam(value = "size", defaultValue = "5") int size,
                                                                 @RequestParam(value = "sort", defaultValue = "id") String sort) {
        List<Product> products = productService.findAllPaginated(page, size, sort);
        return ResponseEntity.ok(products);
    }
